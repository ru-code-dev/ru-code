...
import { MCP_CATALOG_AGGREGATE_ID } from "@t3tools/contracts";
...
} from "../ru-fork/mcp/McpCatalogBuilders.ts";
import {
  findBinding,
...
} from "../ru-fork/mcp/McpInvariants.ts";
import { configIdentity, configPlaceholders } from "@ru-fork/mcp-core";
import { splitServerVars } from "../ru-fork/mcp/McpSecrets.ts";
  ...
  return Crypto.Crypto.pipe(
    Effect.flatMap((crypto) =>
      crypto.randomUUIDv4.pipe(
        Effect.map((eventId) => ({
          eventId: eventId as OrchestrationEvent["eventId"],
...
const decideCommandSequence = Effect.fn("decideCommandSequence")(function* ({
  ...
  ReadonlyArray<PlannedOrchestrationEvent>,
  OrchestrationCommandInvariantError | SecretStoreError | PlatformError.PlatformError,
  ServerSecretStore | Crypto.Crypto
> {
...
> {
  switch (command.type) {
    ...
    // ── ru-fork: MCP catalog commands (aggregate = mcp-catalog) ──────────────
    case "mcp.server-add": {
      yield* requireCatalogServerAbsent({ readModel, command, serverId: command.serverId });
      // ru-fork #2: a server's identity is its config, not its name — reject a duplicate of any
      ...
      // ru-fork #8: every ${VAR} in the config must resolve to a declared var (or ${PROJECT_CWD}).
      {
      ...
      return {
        ...
        payload: { server: buildAddedServer(command.serverId, command.draft, vars, command.createdAt) },
      };
    }
    ...
    case "mcp.server-update": {
      ...
      // ru-fork #2: config-uniqueness on edit — only when a config-affecting field changes. Compute the
    ...
    // ru-fork: migrator → catalog. Add/update a managed built-in by its shipped definition (3-way
    ...
    // ── ru-fork: MCP binding commands (aggregate = project) ───────────────────
    case "mcp.binding-set": {
      ...
      const server = yield* requireCatalogServer({ readModel, command, serverId: command.serverId });
      // ru-fork #8: reject binding varValues whose keys are not declared vars (catches typos that
