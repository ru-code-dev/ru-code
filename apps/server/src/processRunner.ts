...
const runProcessCore = Effect.fn("processRunner.runProcessCore")(function* (
  ...
  // ru-fork: route every spawn through the shared spawn policy.
  // - Windows exe (e.g. git, powershell.exe): shell:false (the upstream #2950
  //   behavior) — avoids the cmd.exe handoff that keeps stdio pipes open.
  // - `--windows-use-bash-for=<bin>` / RU_FORK_WINDOWS_USE_BASH_FOR: bash-route.
  // - shell:true only if a caller explicitly asks for it (input.shell === true).
  const resolved = resolveSpawn(input.command, [...input.args], {
