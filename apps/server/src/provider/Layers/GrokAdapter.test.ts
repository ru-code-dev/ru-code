...
// ru-code: grok is a disabled adapter; its real-process interrupt/cancel/drain
// tests are timing-flaky. Skip the whole suite until grok is re-enabled — remove
// the `describe.skip` wrapper (and its closing `});` at EOF) to restore.
describe.skip("GrokAdapterLive (grok disabled)", () => {
