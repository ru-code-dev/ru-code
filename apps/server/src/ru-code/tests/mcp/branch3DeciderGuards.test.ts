// ru-code: improvements-branch-3 — RED feature tests for the decider-level guards (#2 config
...
describe("branch-3 #2 — catalog config-uniqueness", () => {
  ...
  it("rejects a second server with the SAME config under a different name (RED)", async () => {
    ...
    await system.dispatch(
      addServerCmd({ serverId: "a", name: "Server A", args: ["ctx7"], commandId: "u:a" }),
    );
    // Same config (uvx ctx7), different id + name ⇒ must be rejected once uniqueness lands.
    await expect(
      system.dispatch(
        addServerCmd({ serverId: "b", name: "Server B", args: ["ctx7"], commandId: "u:b" }),
      ),
    ).rejects.toThrow();
    expect(
      (await system.catalog()).filter(
        // ru-code: narrow the stdio|http config union first (ng typechecks these suites).
        (server) => server.config.transport === "stdio" && server.config.args?.[0] === "ctx7",
      ),
    ).toHaveLength(1);
  });
  ...
  it("rejects an edit that makes one server collide with another (RED)", async () => {
    system = makeDeciderSystem();
    await system.dispatch(
      addServerCmd({ serverId: "a", name: "A", args: ["x"], commandId: "u:a" }),
    );
    await system.dispatch(
      addServerCmd({ serverId: "b", name: "B", args: ["y"], commandId: "u:b" }),
    );
  ...
  it("skips a built-in whose config collides with an existing custom server (RED)", async () => {
    system = makeDeciderSystem();
    await system.dispatch(
      addServerCmd({ serverId: "custom", name: "Custom", args: ["demo"], commandId: "u:c" }),
    );
    ...
    expect((await system.catalog()).filter((server) => server.builtinId === "demo")).toHaveLength(
      0,
    );
  });
});
...
describe("branch-3 #8 — varValues / ${VAR} ↔ declared-vars validation", () => {
  ...
  it("rejects a binding whose varValues key is not a declared var (RED)", async () => {
    system = makeDeciderSystem();
    await system.dispatch(
      addServerCmd({ serverId: "s", name: "S", args: ["s"], commandId: "u:s" }),
    );
...
describe("branch-3 #6 — catalog trust flag", () => {
  ...
  it("defaults trust=true on add; an update can turn it off", async () => {
    system = makeDeciderSystem();
    await system.dispatch(
      addServerCmd({ serverId: "t", name: "T", args: ["t"], commandId: "u:t" }),
    );
