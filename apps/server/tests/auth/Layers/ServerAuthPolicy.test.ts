...
import { ServerAuthPolicy } from "../../../src/auth/Services/ServerAuthPolicy.ts";
import { ServerAuthPolicyLive } from "../../../src/auth/Layers/ServerAuthPolicy.ts";
...
it.layer(NodeServices.layer)("ServerAuthPolicyLive", (it) => {
  it.effect("uses desktop-managed-local policy for desktop mode", () =>
    Effect.gen(function* () {
      ...
      // Empty bootstrap surface: the auth gate auto-issues a session for
      // any loopback request in this policy (see auth/Layers/ServerAuth.ts).
