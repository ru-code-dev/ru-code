...
import { readBootstrapEnvelope, resolveFdPath } from "../src/bootstrap.ts";
...
it.layer(NodeServices.layer)("readBootstrapEnvelope", (it) => {
  ...
  it.effect("returns none when the bootstrap read times out before any value arrives", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "atomic-bootstrap-" });
