...
import { ServerEnvironmentLive } from "../../../src/environment/Layers/ServerEnvironment.ts";
...
const makeServerConfig = Effect.fn(function* (baseDir: string) {
  ...
  return {
    ...
    otlpServiceName: "ru-fork-server",
    cwd: process.cwd(),
    baseDir,
    mode: "web",
...
it.layer(NodeServices.layer)("ServerEnvironmentLive", (it) => {
  it.effect("persists the environment id across service restarts", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "ru-fork-server-environment-test-",
      });
  ...
  it.effect("fails instead of overwriting a persisted id when reading the file errors", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "ru-fork-server-environment-read-error-test-",
      });
