...
import { type GitManagerShape } from "../../src/git/GitManager.ts";
import {
  GitHubCli,
  GitHubCliError,
  type GitHubCliShape,
  type GitHubPullRequestSummary,
} from "../../src/sourceControl/GitHubCli.ts";
import {
...
import * as GitHubSourceControlProvider from "../../src/sourceControl/GitHubSourceControlProvider.ts";
...
function makeManager(input?: {
  ...
  // ru-fork: opt into legacy fetch for these tests (production default is 0).
  ...
  const vcsDriverLayer = GitVcsDriver.layer.pipe(
    ...
    // ru-fork: driver now reads `automaticGitFetchInterval` from settings.
...
const GitManagerTestLayer = GitVcsDriver.layer.pipe(
  ...
  // ru-fork: opt into legacy fetch for these tests (production default is 0).
  Layer.provide(
    ServerSettingsService.layerTest({ automaticGitFetchInterval: Duration.seconds(30) }),
  ),
  Layer.provideMerge(VcsProcess.layer),
  Layer.provideMerge(NodeServices.layer),
);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("ru-fork/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("ru-fork/pr-92/main");
