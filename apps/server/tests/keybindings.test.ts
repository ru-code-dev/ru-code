...
it.layer(NodeServices.layer)("keybindings", (it) => {
  ...
  it.effect("ships configurable thread navigation defaults", () =>
    Effect.sync(() => {
      ...
      // ru-fork: model picker keybindings are commented out at the source
      // (packages/shared/src/keybindings.ts) because ru-fork is single-
      // provider and has no model picker UI. Assertions kept commented here so
      // an upstream sync sees the divergence next to the source removal.
