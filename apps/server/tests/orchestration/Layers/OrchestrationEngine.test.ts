...
} from "../../../src/persistence/Services/OrchestrationEventStore.ts";
...
} from "../../../src/orchestration/Services/ProjectionPipeline.ts";
...
describe("OrchestrationEngine", () => {
  ...
  it("records command ack duration using the first committed event type", async () => {
    ...
    expect(
      hasMetricSnapshot(snapshots, "ru_fork_orchestration_command_ack_duration", {
  ...
  it("records failed command dispatches as metric failures", async () => {
    ...
    const snapshots = await system.run(Metric.snapshot);
    expect(
      hasMetricSnapshot(snapshots, "ru_fork_orchestration_commands_total", {
        commandType: "thread.create",
        aggregateKind: "thread",
        outcome: "failure",
      }),
  ...
  it("rolls back all events for a multi-event command when projection fails mid-dispatch", async () => {
    ...
    const flakyProjectionPipeline: OrchestrationProjectionPipelineShape = {
      ...
      projectEvent: (event) => {
        if (
          ...
          event.commandId === CommandId.make("cmd-turn-start-ru-fork") &&
    ...
    await runtime.runPromise(
      engine.dispatch({
        type: "project.create",
        commandId: CommandId.make("cmd-project-ru-fork-create"),
        projectId: asProjectId("project-ru-fork"),
        title: "Ru Fork Project",
        workspaceRoot: "/tmp/project-ru-fork",
        defaultModelSelection: {
          instanceId: ProviderInstanceId.make("codex"),
          model: "gpt-5-codex",
        },
    ...
    await runtime.runPromise(
      engine.dispatch({
        type: "thread.create",
        commandId: CommandId.make("cmd-thread-ru-fork-create"),
        threadId: ThreadId.make("thread-ru-fork"),
        projectId: asProjectId("project-ru-fork"),
        title: "ru-fork",
        modelSelection: {
          instanceId: ProviderInstanceId.make("codex"),
    ...
    const turnStartCommand = {
      ...
      commandId: CommandId.make("cmd-turn-start-ru-fork"),
      threadId: ThreadId.make("thread-ru-fork"),
      message: {
        messageId: asMessageId("msg-ru-fork-1"),
        role: "user" as const,
        text: "hello",
        attachments: [],
      },
