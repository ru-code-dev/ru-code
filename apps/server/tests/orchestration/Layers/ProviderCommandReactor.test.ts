...
import { ProviderAdapterRequestError } from "../../../src/provider/Errors.ts";
...
describe("ProviderCommandReactor", () => {
  ...
  // SKIP — ru-fork only ships CLI — codex-specific model options are not applicable
  it.skip("forwards codex model options through session start and turn send", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        modelSelection: createModelSelection(ProviderInstanceId.make(CLI_NAME), CLI_DEFAULT_MODEL, [
  ...
  // SKIP — ru-fork only ships CLI — claude effort options are not applicable
  ...
  // SKIP — ru-fork only ships CLI — claude fast mode options are not applicable
  ...
  it("starts a first turn on the requested provider instance even when it differs from the thread model", async () => {
    ...
    expect(harness.startSession.mock.calls[0]?.[1]).toMatchObject({
      provider: ProviderDriverKind.make(CLI_NAME),
      providerInstanceId: ProviderInstanceId.make(CLI_NAME),
      modelSelection: {
        instanceId: ProviderInstanceId.make(CLI_NAME),
        model: CLI_DEFAULT_MODEL,
      },
    ...
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.session?.providerName).toBe(CLI_NAME);
    expect(thread?.session?.providerInstanceId).toBe(ProviderInstanceId.make(CLI_NAME));
    expect(
  ...
  // SKIP — ru-fork only ships CLI — codex-specific restart behavior is not applicable
  it.skip("restarts an existing Codex thread on a compatible requested instance", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        modelSelection: {
          instanceId: ProviderInstanceId.make("cli_work"),
          model: CLI_DEFAULT_MODEL,
        },
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        runtimeMode: "approval-required",
    ...
    expect(harness.startSession.mock.calls[1]?.[1]).toMatchObject({
      provider: ProviderDriverKind.make(CLI_NAME),
      providerInstanceId: ProviderInstanceId.make("cli_work"),
      resumeCursor: { opaque: "resume-1" },
    });

    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.session?.providerInstanceId).toBe(ProviderInstanceId.make("cli_work"));
  });
  ...
  // SKIP — ru-fork only ships CLI — claude effort restarts are not applicable
  ...
  // ru-fork: SKIP — runtimeModeChanged was removed from the restart guard
  // in ProviderCommandReactor (see ru-fork-instrumental/analysis/cli-code/
  // 02-setmode-plan-v013-A.md). runtimeMode is now plumbed live via
  // ProviderSendTurnInput.runtimeMode. The inverted-behavior test ("does NOT
  // restart on runtimeMode change") plus broader Clean-1 coverage lives in
  // tests/orchestration/Layers/RuForkProviderCommandReactor.test.ts. Kept here
  // as it.skip so a future upstream sync that restores the old trigger can
  // un-skip the test verbatim.
  it.skip("restarts the provider session when runtime mode is updated on the thread", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  // ru-fork: SKIP — same reason as the runtime-mode restart test above.
  // This test specifically uses `thread.runtime-mode.set` as the restart
  // trigger, which no longer fires a restart. The underlying invariant
  // ("don't stop the active session if restart fails") is still valid for
  // cwd/instance/model triggers — covered by the cwd-change test at line 1034.
  it.skip("does not stop the active session when restart fails before rebind", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  // SKIP — ru-fork only ships CLI — cross-provider switching cannot occur with one driver
  ...
  // SKIP — ru-fork only ships CLI — cross-driver provider changes cannot occur with one driver
  it.skip("rejects cross-driver provider changes after the existing thread session has stopped", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("reacts to thread.approval.respond by forwarding provider approval response", async () => {
    ...
    // ru-fork: runtimeMode added to ProviderRespondToRequestInput so the
    // adapter can pick plan-approval optionId and refresh its currentRuntimeMode
    // mirror at click time. Orchestrator reads thread.runtimeMode at dispatch.
    // This harness's thread.session.set above uses runtimeMode "approval-required".
