...
describe("ProviderRuntimeIngestion", () => {
  ...
  it.skip("buffers assistant deltas by default until completion", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it.skip("does not create assistant segments for whitespace-only buffered text at approval boundaries", async () => {
    const harness = await createHarness();
  ...
  it("projects compacted thread state into context compaction activities", async () => {
    ...
    expect(activity?.summary).toBe("Контекст уплотнён");
  ...
  it("projects Codex task lifecycle chunks into thread activities", async () => {
    ...
    expect(started?.summary).toBe("Запущена задача плана");
