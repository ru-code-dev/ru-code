...
describe("ProcessRunner.run", () => {
  ...
  it.effect("wraps spawn failures in ProcessSpawnError", () =>
    Effect.gen(function* () {
      const spawner = makeSpawner(
        () =>
          Effect.fail(
            new Error("spawn ENOENT"),
          ) as Effect.Effect<ChildProcessSpawner.ChildProcessHandle>,
      );
      ...
      expect(error).toBeInstanceOf(ProcessRunner.ProcessSpawnError);
    }),
  );
...
describe("isWindowsCommandNotFound", () => {
  it("matches the localized German cmd.exe error text", () => {
    ...
    try {
      expect(
        ProcessRunner.isWindowsCommandNotFound(
          1,
  ...
  it("treats Windows exit code 9009 as command-not-found", () => {
    const originalPlatform = process.platform;
    Object.defineProperty(process, "platform", { value: "win32", configurable: true });

    try {
      expect(ProcessRunner.isWindowsCommandNotFound(9009, "")).toBe(true);
    } finally {
      Object.defineProperty(process, "platform", { value: originalPlatform, configurable: true });
    }
  });
