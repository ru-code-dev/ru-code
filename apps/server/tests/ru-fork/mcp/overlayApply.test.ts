// ru-fork: behavior tests for the qwen settings overlay (McpOverlay.writeOverlay / removeOverlay) —
// the file qwen actually reads. Drives the REAL engine to populate catalog + bindings, writes the
// overlay to a temp dir, reads it back, and asserts the JSON qwen will see: which servers are
// included (enabled / not-disabled / complete), the stdio/http entry shape, secret materialization
// into env, tool-policy include/excludeTools, folderTrust off, and the empty-overlay / remove paths.
...
function projectCreate(projectId: string, workspaceRoot: string, commandId: string): OrchestrationCommand {
  return {
    type: "project.create",
...
function addStdio(
  ...
  return {
    ...
    draft: { name, config: { transport: "stdio", command: "uvx", args: [...args] }, vars, timeoutMs: null },
    createdAt: ISO,
  };
}
...
function setServerEnabled(serverId: string, enabled: boolean, commandId: string): OrchestrationCommand {
  return {
...
function bind(
  ...
  return {
    ...
    projectId: ProjectId.make(projectId),
    serverId: McpServerId.make(serverId),
    patch: { enabled: patch.enabled ?? true, ...(patch.varValues ? { varValues: patch.varValues } : {}), ...(patch.toolPolicy ? { toolPolicy: patch.toolPolicy } : {}) },
  };
}
...
function makeSystem() {
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), { prefix: "t3-mcp-overlay-test-" });
    ...
    writeOverlay: (projectId: string) =>
      runtime.runPromise(
        Effect.flatMap(Effect.service(McpOverlay), (overlay) =>
          overlay.writeOverlay(ProjectId.make(projectId)),
        ),
      ),
...
describe("McpOverlay.writeOverlay — the qwen settings file", () => {
  ...
  it("writes an enabled stdio binding with resolved command/args/env/cwd/timeout + folderTrust off", async () => {
    ...
    await system.dispatch(
      addStdio("s-fs", "fs", ["--root", "${PROJECT_CWD}"], [], "add:fs"),
    );
  ...
  it("materializes a per-project secret value into the entry's env", async () => {
    ...
    const env = (await system.readOverlay(result.overlayPath)).mcpServers["s-sec"].env as Record<string, string>;
  ...
  it("excludes a DISABLED binding, a CATALOG-disabled server, and an INCOMPLETE binding", async () => {
    ...
    // ru-fork #2: distinct configs (identity is the config) — each server uses different args.
    // (a) disabled binding
    ...
    await system.dispatch(
      addStdio("s-inc", "inc", ["inc"], [{ name: "NEED", secret: false, perProject: true, required: true, value: null }], "add:inc"),
    );
  ...
  it("writes an http binding as httpUrl/headers with ${VAR} expanded", async () => {
    ...
    await system.dispatch(
      addHttp(
        ...
        [
          { name: "HOST", secret: false, perProject: false, required: true, value: "api.example.com" },
          { name: "TOKEN", secret: true, perProject: true, required: true, value: null },
        ],
        "add:http",
      ),
  ...
  it("derives include/excludeTools from the tool POLICY (not discovered tools)", async () => {
    ...
    // ru-fork #2: distinct configs (identity is the config) — args differ so both servers can be added.
    await system.dispatch(addStdio("s-deny", "deny", ["deny"], [], "add:deny"));
    await system.dispatch(
      bind("p5", "s-deny", { toolPolicy: { defaultDecision: "deny", exceptions: ["read"] } }, "bind:deny"),
    );
    await system.dispatch(addStdio("s-allow", "allow", ["allow"], [], "add:allow"));
    await system.dispatch(
      bind("p5", "s-allow", { toolPolicy: { defaultDecision: "allow", exceptions: ["danger"] } }, "bind:allow"),
    );
    ...
    // ru-fork #6: every server entry carries qwen's `trust` flag (default true).
  ...
  // ── ru-fork #4: ephemeral-overlay leaf ops (used by the spawn finalizer + sweeps) ──
