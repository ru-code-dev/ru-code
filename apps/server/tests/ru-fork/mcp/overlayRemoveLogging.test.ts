// ru-fork: McpOverlay.removeOverlay error-path. It runs `fileSystem.remove(overlayDir, {recursive})`
...
describe("McpOverlay.removeOverlay error path", () => {
  ...
  it("does not fail the caller, but LOGS when removing a project's overlay fails", async () => {
    ...
    const overlayLog = system.captured.find((entry) => entry.message.toLowerCase().includes("overlay"));
