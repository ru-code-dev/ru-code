// ru-fork: behavior tests for McpProjectionQuery.getSnapshot — the client-facing catalog+bindings
...
import { McpProjectionQuery } from "../../../src/ru-fork/mcp/McpProjectionQuery.ts";
import { McpProjectionQueryLive } from "../../../src/ru-fork/mcp/McpProjectionQuery.ts";
...
const unbind = (p: string, s: string, c: string): OrchestrationCommand => ({ type: "mcp.binding-remove", commandId: CommandId.make(c), projectId: ProjectId.make(p), serverId: McpServerId.make(s) });
...
function makeSystem() {
  ...
  return {
    dispatch: (cmd: OrchestrationCommand) =>
      runtime.runPromise(Effect.flatMap(Effect.service(OrchestrationEngineService), (e) => e.dispatch(cmd))),
