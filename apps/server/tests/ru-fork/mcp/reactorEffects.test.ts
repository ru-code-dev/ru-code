// ru-fork: behavior tests for the reactor's extracted effects driven against the REAL engine —
...
function projectCreate(projectId: string, cmd: string): OrchestrationCommand {
  return { type: "project.create", commandId: CommandId.make(cmd), projectId: ProjectId.make(projectId), title: "P", workspaceRoot: `/w/${projectId}`, createdAt: ISO };
}
function addStdio(serverId: string, args: ReadonlyArray<string>, vars: ReadonlyArray<McpServerVarDraft>, cmd: string): OrchestrationCommand {
  return { type: "mcp.server-add", commandId: CommandId.make(cmd), serverId: McpServerId.make(serverId), draft: { name: serverId, config: { transport: "stdio", command: "uvx", args: [...args] }, vars, timeoutMs: null }, createdAt: ISO };
}
function setEnabled(serverId: string, enabled: boolean, cmd: string): OrchestrationCommand {
  return { type: "mcp.server-update", commandId: CommandId.make(cmd), serverId: McpServerId.make(serverId), patch: { enabled } };
}
function remove(serverId: string, cmd: string): OrchestrationCommand {
  return { type: "mcp.server-remove", commandId: CommandId.make(cmd), serverId: McpServerId.make(serverId) };
}
function bind(projectId: string, serverId: string, varValues: Record<string, string>, cmd: string): OrchestrationCommand {
  return { type: "mcp.binding-set", commandId: CommandId.make(cmd), projectId: ProjectId.make(projectId), serverId: McpServerId.make(serverId), patch: { enabled: true, varValues } };
}
...
function makeSystem(autobindDefaults: boolean) {
  ...
  return {
    ...
    secretGet: (ref: string) =>
      runtime.runPromise(Effect.flatMap(Effect.service(ServerSecretStore), (s) => s.get(ref))),
...
describe("computeDesiredEffect — probe-target selection", () => {
  ...
  it("excludes a catalog-disabled server and an incomplete binding", async () => {
    ...
    await system.dispatch(
      addStdio("s-inc", ["${NEED}"], [{ name: "NEED", secret: false, perProject: true, required: true, value: null }], "add:inc"),
    );
    const desired = await system.computeDesired();
  ...
  it("two bindings with DIFFERENT per-project values are two instances; SAME value collapses to one", async () => {
    ...
    await system.dispatch(
      addStdio("s-v", ["${ARG}"], [{ name: "ARG", secret: false, perProject: true, required: true, value: null }], "add:v"),
    );
...
describe("gcOrphanedSecretsEffect — secret GC", () => {
  ...
  it("prunes a secret no longer referenced, keeps a live one", async () => {
    const secretVar: McpServerVarDraft = { name: "TOK", secret: true, perProject: false, required: false, value: "v" };
