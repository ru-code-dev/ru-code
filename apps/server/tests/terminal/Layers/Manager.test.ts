...
import {
  ...
  type PtySpawnInput,
  PtySpawnError,
} from "../../../src/terminal/Services/PTY.ts";
import { makeTerminalManagerWithOptions } from "../../../src/terminal/Layers/Manager.ts";
...
it.layer(NodeServices.layer, { excludeTestServices: true })("TerminalManager", (it) => {
  ...
  it.effect("prefers Git Bash for Windows terminals", () =>
    Effect.gen(function* () {
      const { manager, ptyAdapter } = yield* createManager(5, {
        platform: "win32",
        env: {
  ...
  it.effect("falls back to built-in Git Bash by absolute path on Windows", () =>
    Effect.gen(function* () {
      const { manager, ptyAdapter } = yield* createManager(5, {
        platform: "win32",
        env: {
      ...
      ptyAdapter.spawnFailures.push(
        ...
        new Error("spawn bash.exe ENOENT"),
      );
      ...
      expect(ptyAdapter.spawnInputs[1]?.args).toEqual(["--login", "-i"]);
      expect(ptyAdapter.spawnInputs[2]?.args).toEqual(["--login", "-i"]);
    }),
  );
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(spawnInput.env.RU_FORK_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            RU_FORK_PROJECT_ROOT: "/repo",
            RU_FORK_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.RU_FORK_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.RU_FORK_WORKTREE_PATH, "/repo/worktree-a");
