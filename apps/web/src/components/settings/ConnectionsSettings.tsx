...
function AccessScopeSummary({
  ...
  const scopeCountLabel = L(
    `${scopes.length} ${scopes.length === 1 ? "scope" : "scopes"}`,
    `${scopes.length} ${pluralRu(scopes.length, ["область", "области", "областей"])}`,
  );
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description={`Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. ${APP_NAME} restarts when you change this.`}
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? `${APP_NAME} will restart to open this environment over the network.`
                    : `${APP_NAME} will restart and restrict this environment to this computer again.`}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? `${APP_NAME} will restart on the Windows backend. Threads and projects open in WSL stay safe inside the distribution and become available again when you enable WSL.`
                      : `The WSL backend will be stopped. Threads and projects open in WSL stay safe inside the distribution but are unavailable in ${APP_NAME} until you enable WSL again.`
                    : pendingWslChange?.kind === "distro"
                      ? `${APP_NAME} will restart the WSL backend on the new distribution. Sessions still running on the current distribution will be interrupted.`
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? `${APP_NAME} will restart and run only the WSL backend. Your Windows-side projects will be unavailable until you turn this off again.`
                          : `${APP_NAME} will restart and bring the Windows backend back up alongside WSL.`}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {`${APP_NAME} will restart the local backend without Tailscale Serve.`}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  {`${APP_NAME} will restart the local backend with Tailscale Serve enabled and ask Tailscale to proxy HTTPS traffic to this backend.`}
                </DialogDescription>
              </DialogHeader>
              <DialogPanel className="space-y-4">
                <label className="block">
