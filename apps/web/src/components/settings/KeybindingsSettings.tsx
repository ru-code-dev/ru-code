  ...
  return (
    <Tooltip>
      ...
      <TooltipPopup side="top" className="max-w-72 whitespace-normal leading-relaxed">
        {`${APP_NAME} doesn't recognize this condition yet. You can still save it, but it may not fire until the runtime provides it.`}
      </TooltipPopup>
    </Tooltip>
  );
}
      ...
      >
        {!isElectron ? (
          <div className="flex items-start gap-2 border-b border-warning/20 bg-warning/5 px-3 py-2.5 text-[12px] leading-relaxed text-muted-foreground sm:px-4">
            ...
            <p>
              {`Some shortcuts may be claimed by the browser before ${APP_NAME} sees them. Use the desktop app for better keybinding support.`}
            </p>
          </div>
        ) : null}
