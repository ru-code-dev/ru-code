...
test.describe("service worker navigate-fallback (SW-down test)", () => {
  ...
  test("no marker: a failed navigation serves the branded down page, and returns when back (#40)", async ({
    ...
    try {
      // RETRY THE PRECONDITION, not the claim — see the note on the W15 spec
      // below: `context.setOffline(true)` does not reliably reach the SERVICE
      // WORKER's own `fetch(event.request.url)` (sw.ts:155), so a reload can
      // simply succeed and never exercise the fallback at all.
      await expect(async () => {
        await page.reload({ waitUntil: "domcontentloaded" }).catch(() => {});
        await expect(page.getByTestId("sw-down-page")).toBeVisible({ timeout: 8_000 });
  ...
  test("a fresh marker: the failed navigation shows the updating page instead (W15) (#40)", async ({
    ...
    await context.setOffline(true);
    try {
      // ru-code (ap-final T4): THE FIX FOR THIS SPEC'S LONG-STANDING FLAKE.
      //
      // The claim — "a FAILED navigation renders the updating page" — was never
      // the flaky part. Its PRECONDITION was. Playwright's
      // `context.setOffline(true)` drives CDP network emulation on the page
      // target, and the request that has to fail here is issued by the SERVICE
      // WORKER (`fetch(event.request.url)`, sw.ts:155), which is a separate
      // target that does not reliably inherit it. When it does not, the reload
      // simply SUCCEEDS and neither fallback page is ever rendered — which is
      // exactly what the failing run recorded: the call log ends
      // `navigated to ".../settings/auto-update"`, the real app, with
      // `getByTestId('sw-updating-page')` never found
      // (`logs/ap-final/final_e2e2.log`).
      //
      // Retrying the reload retries the precondition and nothing else: the
      // assertion inside is unchanged and still has to be satisfied by the SW.
      //
      // A previous hypothesis — that `clearMarker`'s absence-poll let a queued
      // Cache delete land after the marker write — was implemented and then
      // REVERTED: the flake reproduced with it in place, so it was not the
      // cause. Recorded so it is not re-tried.
      await expect(async () => {
        await page.reload({ waitUntil: "domcontentloaded" }).catch(() => {});
        await expect(page.getByTestId("sw-updating-page")).toBeVisible({ timeout: 8_000 });
      }).toPass({ timeout: 60_000 });
      await expect(page.getByTestId("sw-down-page")).toHaveCount(0);
    } finally {
      await context.setOffline(false);
